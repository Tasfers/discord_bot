...
class PersistentViewBot(commands.Bot): #создание активности бота
    def __init__(self): 
    ...
    async def on_ready(self): 
        if not self.persistent_views_added:
            ...
            self.persistent_views_added = True #это отвечает за включение активности у бота("играет в" и.т.п)
        print(f"=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\nБот запущен\nName: {bot.user.name}#{bot.user.discriminator}\nID: {bot.user.id}\n=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=") #тут бот принтует в консоль при запуске, можно написать что угодно

bot = PersistentViewBot()

for filename in os.listdir("./cogs"): #это отвечает за загрузку когов, эти 3 строки не трогать
    if filename.endswith(".py"):
        bot.load_extension("cogs." + filename[:-3])

@bot.remove_command("help") #это удаляет стандартную и не красивую команду help
        
@bot.slash_command(description='Загрузить модуль бота') #эта slash команда отвечает за загрузку когов
@commands.is_owner()
...
@bot.slash_command(description='Выгрузить модуль бота') #эта slash команда отвечает за выгрузку когов (отключение)
@commands.is_owner()
...
@bot.slash_command(description="Перезагрузить модуль бота") #эта slash команда отвечает за перезагрузку когов (т.е не обязательно перезагружать бота, можно перезагрузить сам ког)
@commands.is_owner()
